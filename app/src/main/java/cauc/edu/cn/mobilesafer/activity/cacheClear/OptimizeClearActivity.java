        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case UPDATE_CATCH_APP:
                    ...
                    //将实时扫描到的应用缓存添加到滚动显示列表
                    mOptimizeAppLayout.addView(view, 0);
                    break;
                case CHECK_CACHE_APP:
                    //无论是否检测到缓存，均将当前扫描的应用的名称显示出来，以示进度
                    mScanningAppName.setText((String) msg.obj);
                    break;
                case CHECK_FINISH:
                    mScanningAppName.setText("扫描完成");
                    AlertDialog.Builder builder = new AlertDialog.Builder(OptimizeClearActivity.this);
                    builder.setMessage("目前只有Android系统应用才能清理缓存！")
                            .setCancelable(false)
